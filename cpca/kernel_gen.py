...
class pinv_k_nn_laplacian_kernel(kernel_functional):
    ...
    def compute_matrix(self, points, params={'sigma' : 1.0, 'k' : 3}):
        adj_mat = float (0.5 / np.power(params['sigma'], 2)) * utils.k_nn_pw_sq_distance_mat(points, params['k'])
        adj_mat = np.exp(-adj_mat)

        adj_mat_normalizer = np.diag(1.0 / np.sqrt(np.sum(adj_mat, axis=0)))
        laplacian_mat = np.identity(adj_mat.shape[0]) - np.dot(np.dot(adj_mat_normalizer, adj_mat), adj_mat_normalizer)
        
        pinv_laplacian_mat = np.linalg.pinv(laplacian_mat) + 1e-6 * np.identity(adj_mat.shape[0])
    ...
    def evaluate(self, x, y, params=None):
        raise ValueError('Not supported! You must compute the full kernel matrix as the dot product depends on all data points!')
...
class pinv_laplacian_kernel(kernel_functional):
    ...
    def compute_matrix(self, points, params=None):
        n = points.shape[0]
        edist_adj_mat = utils.pairwise_distance_mat(points)
        if None == params or None == params['sigma']:
            g = utils.sim_mat_to_graph(edist_adj_mat)
            mst_result = utils.min_spanning_tree(g, 0)
        ...
        gauss_adj_mat = utils.prune_kernel_mat(np.exp(float(-0.5 / np.power(gauss_sigma, 2)) * np.power(edist_adj_mat, 2)), params['epsilon'])
        adj_deg_normalizer = np.diag(1. / np.sqrt(np.sum(gauss_adj_mat, axis=0)))
        laplacian_mat = np.identity(n) - np.dot(np.dot(adj_deg_normalizer, gauss_adj_mat), adj_deg_normalizer)
