...
def get_log_file_handler(fileName, maxCapacity, backups, level):
    handler = handlers.RotatingFileHandler(fileName, maxBytes=maxCapacity, backupCount=backups)
...
def is_soft_orthogonal(logger, alphas, K, threshold = 0.95):
    ...
    for i in range(d):
        for j in range(i + 1, d):
            ...
            logger.debug('< f_{' + str(i) + '}, f_{' + str(j) + '} > = ' + str(orth_coeff))
...
#    f.close()
    ...
    async_results = [processor_pool.apply_async(min_spanning_tree, (g, i)) for i in range(n)]
...
def variance_weighting(sh_paths, pos_lab_indices, neg_lab_indices):
    ...
    for i in range(sh_paths.shape[0]):
        dist_to_pos = float(np.sum(sh_paths[i, pos_lab_indices]) / pos_lab_indices.shape[0])
        dist_to_neg = float(np.sum(sh_paths[i, neg_lab_indices]) / neg_lab_indices.shape[0])
